...
template <class ExtentsType, ExtentsType... E> struct basic_static_extents
{
    ...
    static_assert(!std::numeric_limits<value_type>::is_signed,
                  "Static error in basic_static_extents: type must be of type unsigned integer.");
...
template <std::size_t... E> using static_extents = basic_static_extents<std::size_t, E...>;

template <typename T> struct static_product;

template <typename T> inline static constexpr auto const static_product_v = static_product<T>::value;
...
template <typename ExtentsType, ExtentsType E0> struct static_product<basic_static_extents<ExtentsType, E0>>
{
    static constexpr auto const value = E0;
};

template <typename ExtentsType> struct static_product<basic_static_extents<ExtentsType>>
{
...
template <typename T, typename E, typename F> struct tensor_traits<static_tensor<T, E, F>>
{
