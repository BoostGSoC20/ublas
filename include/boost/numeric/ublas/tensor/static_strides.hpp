...
template <typename T, T... N1, T... N2> struct concat<static_stride_list<T, N1...>, static_stride_list<T, N2...>>
{
...
template <typename T, std::size_t N> using make_sequence_of_ones_t = typename make_sequence_of_ones<T, N>::type;
...
template <typename T, std::size_t N> using make_sequence_of_ones_t = impl::make_sequence_of_ones_t<T, N>;
...
constexpr auto make_static_strides_helper(static_stride_list<T, E0, E...>, static_stride_list<T, R...>,
                                          static_stride_list<T, P...>)
{
...
template <typename L, typename T, T E0, T... E> constexpr auto make_static_strides(static_stride_list<T, E0, E...>)
{
    ...
    if constexpr (!(static_traits::is_scalar_v<extents_type> || static_traits::is_vector_v<extents_type>))
    {
        ...
        else
        {
            ...
            if constexpr (std::is_same_v<L, first_order>)
            {
                ...
                using ret_type = decltype(make_static_strides_helper<L>(
                    static_stride_list<T, E0, E...>{}, static_stride_list<T>{}, static_stride_list<T>{}));
            ...
            else
            {
                ...
                using ret_type = decltype(make_static_strides_helper<L>(
                    static_stride_list<T, E...>{}, static_stride_list<T>{}, static_stride_list<T>{}));
...
template <typename L, typename T> constexpr auto make_static_strides(static_stride_list<T>)
{
 ...
 *
 */
template <class Layout, class T, T... Extents> struct basic_static_strides<basic_static_extents<T, Extents...>, Layout>
{
    ...
    [[nodiscard]] inline constexpr const_reference operator[](size_type k) const noexcept
    {
        return m_data[k];
    }
    ...
    constexpr basic_static_strides() noexcept
    {
        static_assert(_size == 0 || (static_traits::is_valid_v<extents_type> &&
                      ...
                      "Size can be 0 or Shape should be valid and shape can be vector or shape can be scalar or size "
    ...
    constexpr basic_static_strides &operator=(basic_static_strides const &other) noexcept = default;
