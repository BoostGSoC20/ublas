...
struct static_tensor : public basic_tensor<static_tensor<T, E, F>>
{
    ...
    {
        static_assert(extents_type::_size == 2, "Error in boost::numeric::ublas::tensor(const matrix_type &v)"
    ...
    constexpr static_tensor(const vector_type &v)
    {
        ...
        static_assert(extents_type::_size == 2, "Error in boost::numeric::ublas::tensor(const vector_type &v)"
                                                " : invalid extents size");
    ...
    template <typename OtherTensor> constexpr static_tensor(const basic_tensor<OtherTensor> &other)
    {
        static_assert(
        ...
        if (other.extents() != super_type::extents())
        {
            throw std::runtime_error(
                "boost::numeric::ublas::static_tensor( const OtherTensor& ) : The tensor should have equal extents");
        }

        std::copy(other.begin(), other.end(), super_type::begin());
    }

    template <typename OtherTensor> constexpr static_tensor(basic_tensor<OtherTensor> &&other)
    {
        static_assert(
            is_valid_tensor_v<OtherTensor>,
        ...
        {
            throw std::runtime_error(
                "boost::numeric::ublas::static_tensor( OtherTensor&& ) : The tensor should have equal extents");
        }
    ...
    template <typename derived_type>
    constexpr static_tensor(const tensor_expression_type<derived_type> &expr) : super_type(expr)
    {
    }

    template <typename derived_type>
    inline constexpr static_tensor &operator=(const tensor_expression_type<derived_type> &expr)
    {
    ...
    template <typename derived_type>
    constexpr static_tensor(const matrix_expression_type<derived_type> &expr) : static_tensor(matrix_type(expr))
    {
    }

    template <typename derived_type>
    constexpr static_tensor(const vector_expression_type<derived_type> &expr) : static_tensor(vector_type(expr))
    {
    }
