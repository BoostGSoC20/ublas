...
template <typename E> constexpr auto extents_result_tensor_times_vector(E const &)
{
    static_assert(
        is_static_rank<E>::value,
...
template <typename T> constexpr auto extents_result_tensor_times_vector(basic_extents<T> const &e)
{
...
template <typename E> constexpr auto extents_result_tensor_times_matrix(E const &a)
{
    static_assert(
        is_static_rank<E>::value,
...
template <typename T> constexpr auto extents_result_tensor_times_matrix(basic_extents<T> const &e)
{
    return dynamic_extents<>{e};
}

template <typename T, T... E1, T... E2>
constexpr auto extents_result_type_outer_prod(basic_static_extents<T, E1...> const &,
                                              basic_static_extents<T, E2...> const &)
{
    return dynamic_extents<sizeof...(E1) + sizeof...(E2)>();
}

template <typename T, T... E, size_t R>
constexpr auto extents_result_type_outer_prod(basic_static_extents<T, E...> const &,
                                              basic_fixed_rank_extents<T, R> const &)
{
    return dynamic_extents<sizeof...(E) + R>();
}

template <typename T, T... E, size_t R>
constexpr auto extents_result_type_outer_prod(basic_fixed_rank_extents<T, R> const &,
                                              basic_static_extents<T, E...> const &)
{
    return dynamic_extents<R + sizeof...(E)>();
}

template <typename E1, typename E2> auto extents_result_type_outer_prod(E1 const &e1, E2 const &e2)
{
...
template <typename T> inline static constexpr bool is_complex_v = is_complex<T>::value;
                           ...
                           const std::size_t m)
{
    ...
    if (p < m)
        throw std::length_error("error in boost::numeric::ublas::prod(ttv): rank of tensor must be "
    ...
    auto bb = &(b(0));
    ttv(m, p, c.data(), c.extents().data(), c.strides().data(), a.data(), a.extents().data(), a.strides().data(), bb,
        nb.data(), nb.data());
    return c;
}
                           ...
                           const std::size_t m)
{
    ...
    auto bb = &(b(0, 0));
    ttm(m, p, c.data(), c.extents().data(), c.strides().data(), a.data(), a.extents().data(), a.strides().data(), bb,
        nb.data(), wb.data());
                           ...
                           std::vector<std::size_t> const &phia, std::vector<std::size_t> const &phib)
{
    ...
    if (pa < q)
        throw std::runtime_error("error in ublas::prod: number of contraction dimensions cannot be greater than the "
                                 "order of the left-hand side tensor.");
    if (pb < q)
        throw std::runtime_error("error in ublas::prod: number of contraction dimensions cannot be greater than the "
                                 "order of the right-hand side tensor.");

    if (q != phib.size())
        throw std::runtime_error("error in ublas::prod: permutation tuples must have the same length.");

    if (pa < phia.size())
        throw std::runtime_error("error in ublas::prod: permutation tuple for the left-hand side tensor cannot be "
                                 "greater than the corresponding order.");
    if (pb < phib.size())
        throw std::runtime_error("error in ublas::prod: permutation tuple for the right-hand side tensor cannot be "
                                 "greater than the corresponding order.");
    ...
    auto c = dynamic_tensor<value_type, layout_type>(dynamic_extents<>(nc),
                                                     value_type{}); // tensor(dynamic_extents<>(nc), value_type{});

    ttt(pa, pb, q, phia1.data(), phib1.data(), c.data(), c.extents().data(), c.strides().data(), a.data(),
        a.extents().data(), a.strides().data(), b.data(), b.extents().data(), b.strides().data());
...
inline decltype(auto) inner_prod(basic_tensor<TensorType1> const &a, basic_tensor<TensorType2> const &b)
{
    ...
    return inner(a.rank(), a.extents().data(), a.data(), a.strides().data(), b.data(), b.strides().data(),
                 value_type{0});
}
 ...
 */
template <typename TensorType1, typename TensorType2,
          std::enable_if_t<!(is_static<typename TensorType1::extents_type>::value &&
...
inline decltype(auto) outer_prod(basic_tensor<TensorType1> const &a, basic_tensor<TensorType2> const &b)
{
    ...
    outer(c.data(), c.rank(), c.extents().data(), c.strides().data(), a.data(), a.rank(), a_extents.data(),
          a.strides().data(), b.data(), b.rank(), b_extents.data(), b.strides().data());
...
{
    static_assert(is_valid_tensor_v<TensorType>, "boost::numeric::ublas::trans() : tensor type should be valid tensor");
 ...
 */
template <typename TensorType> inline decltype(auto) norm(basic_tensor<TensorType> const &a)
{

    static_assert(is_valid_tensor_v<TensorType>, "boost::numeric::ublas::norm() : tensor type should be valid tensor");
...
{
    return detail::make_unary_tensor_expression<basic_tensor<TensorType>>(expr(),
 ...
 */
template <class T, class D> auto conj(detail::tensor_expression<T, D> const &expr)
{
    using tensor_type = T;
    using value_type = typename tensor_type::value_type;
 ...
 * @returns   unary tensor expression
 */
template <class T, class D> auto real(detail::tensor_expression<T, D> const &expr)
{
 ...
 * @returns   unary tensor expression
 */
template <class T, class D> auto imag(detail::tensor_expression<T, D> const &lhs)
{
...
inline constexpr auto extents_result_tensor_times_vector(basic_static_extents<T>, basic_static_extents<T, E...>,
...
inline constexpr auto extents_result_tensor_times_vector(basic_static_extents<T, E0, E...>,
                                                         basic_static_extents<T, O0, OtherE...>,
                                                         basic_static_extents<T, R...> = basic_static_extents<T>{})
{
    if constexpr (I != M - 1)
    {
...
inline constexpr auto extents_result_tensor_times_vector(basic_static_extents<T, E0, E...> const &e)
{
...
inline constexpr auto static_extents_set_at(basic_static_extents<T, E0, E...> const &,
                                            basic_static_extents<T, OtherE...> = basic_static_extents<T>{})
{
    ...
    else
    {
        ...
        {
            return static_extents_set_at<I - 1, NE>(basic_static_extents<T, E...>{},
...
namespace impl
{
template <typename T, T... E1, T... E2> struct concat<basic_static_extents<T, E1...>, basic_static_extents<T, E2...>>
{
...
inline decltype(auto) prod(basic_tensor<TensorType> const &a, vector<typename TensorType::value_type, A> const &b)
{
    ...
    static_assert(M != 0, "error in boost::numeric::ublas::prod(ttv): "
                          "contraction mode must be greater than zero.");

    static_assert(extents_type::_size >= M, "error in boost::numeric::ublas::prod(ttv): rank of tensor must be "
                                            "greater than or equal to the modus.");

    static_assert(extents_type::_size != 0, "error in boost::numeric::ublas::prod(ttv): first "
                                            "argument tensor should not be empty.");
    ...
    ttv(M, p, c.data(), c_static_extents.data(), c_static_strides.data(), a.data(), a_static_extents.data(),
        a_static_strides.data(), bb, nb.data(), nb.data());
                           ...
                           matrix<typename TensorType::value_type, typename TensorType::layout_type, A> const &b)
{
    ...
    static_assert(M != 0, "error in boost::numeric::ublas::prod(ttm): "
                          "contraction mode must be greater than zero.");

    static_assert(extents_type::_size >= M, "error in boost::numeric::ublas::prod(ttm): rank "
                                            "of the tensor must be greater equal the modus.");

    static_assert(extents_type::_size, "error in boost::numeric::ublas::prod(ttm): first "
                                       "argument tensor should not be empty.");
    ...
    auto &a_static_strides = a.strides().base();
    auto &c_static_strides = c.strides().base();
    ttm(M, p, c.data(), c_static_extents.data(), c_static_strides.data(), a.data(), a_static_extents.data(),
        a_static_strides.data(), bb, nb.data(), wb.data());
...
inline decltype(auto) outer_prod(basic_tensor<TensorType1> const &a, basic_tensor<TensorType2> const &b)
{
    ...
    outer(c.data(), c.rank(), c_static_extents.data(), c_static_strides.data(), a.data(), a.rank(),
          a_static_extents.data(), a_static_strides.data(), b.data(), b.rank(), b_static_extents.data(),
