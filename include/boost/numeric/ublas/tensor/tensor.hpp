...
template <class T> class basic_tensor : public detail::tensor_expression<basic_tensor<T>, basic_tensor<T>>
{
  ...
  public:
    ...
    static_assert(std::is_same<layout_type, first_order>::value || std::is_same<layout_type, last_order>::value,
  ...
  protected:
    ...
    constexpr basic_tensor(extents_type const &e, dynamic_tensor_tag)
        : tensor_expression_type<self_type>(), extents_(e), strides_(extents_), data_(product(extents_))
    {
    }

    constexpr basic_tensor(extents_type const &e, static_tensor_tag)
        : tensor_expression_type<self_type>(), extents_(e), strides_(extents_)
    {
    }
     ...
     */
    explicit inline basic_tensor(std::initializer_list<size_type> l) : basic_tensor(std::move(l), container_tag{})
    {
    }
     ...
     */
    explicit inline basic_tensor(extents_type const &s) : basic_tensor(s, container_tag{})
    {
    }
     ...
     */
    explicit inline basic_tensor(extents_type const &s, value_type const &i) : basic_tensor(s, container_tag{})
    {
     ...
     */
    basic_tensor(extents_type const &s, const array_type &a) : basic_tensor(s, container_tag{})
    {
        if (product(extents_) != a.size())
        {
            throw std::runtime_error(
                "boost::numeric::ublas::basic_tensor(extents_type,array_type): array size mismatch with extents");
        }
  ...
  public:
        ...
        : basic_tensor(detail::retrieve_extents(expr), container_tag{})
    {
        static_assert(detail::has_tensor_types<self_type, tensor_expression_type<derived_type>>::value,
                      "Error in boost::numeric::ublas::basic_tensor: expression does not contain a basic_tensor. "
                      "cannot retrieve shape.");
    ...
    basic_tensor(const matrix_expression_type<derived_type> &expr) : basic_tensor(matrix_type(expr))
    {
    }
     ...
     */
    template <class derived_type>
    basic_tensor(const vector_expression_type<derived_type> &expr) : basic_tensor(vector_type(expr))
    {
    }
     ...
     */
    inline basic_tensor(const basic_tensor &v)
        : tensor_expression_type<self_type>(), extents_(v.extents_), strides_(v.strides_), data_(v.data_)
    {
    }
     ...
     */
    inline basic_tensor(basic_tensor &&v)
        : tensor_expression_type<self_type>() // tensor_container<self_type> ()
          ,
          extents_(std::move(v.extents_)), strides_(std::move(v.strides_)), data_(std::move(v.data_))
    {
    }
     ...
     */
    template <class derived_type> basic_tensor &operator=(const tensor_expression_type<derived_type> &expr)
    {
        detail::eval(*this, expr);
        return *this;
    }
    ...
    inline strides_type const &strides() const
    {
        return this->strides_;
    }

    /** @brief Returns the extents of the basic_tensor */
    inline extents_type const &extents() const
    {
     ...
     */
    template <class... size_types> inline const_reference at(size_type i, size_types... is) const
    {
     ...
     *  @param is zero-based indices where 0 <= is[r] < this->size(r) where  0 < r < this->rank()
     */
    template <class... size_types> inline reference at(size_type i, size_types... is)
    {
        if constexpr (sizeof...(is) == 0)
            return this->data_[i];
        else
    ...
    inline decltype(auto) operator()(index::index_type<I> p, index_types... ps) const
    {
        constexpr auto N = sizeof...(ps) + 1;
        if (N != this->rank())
            throw std::runtime_error("Error in boost::numeric::ublas::operator(): size of provided index_types does "
                                     "not match with the rank.");
    ...
    /// \brief Return a const reverse iterator before the first element of the reversed basic_tensor (i.e. end() of
    /// normal basic_tensor)
...
{
template <typename T, typename... Ts> struct tensor_rebind<basic_tensor<T>, Ts...>
{
