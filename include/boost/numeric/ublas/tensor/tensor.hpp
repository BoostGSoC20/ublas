...
public:
  ...
  static_assert(std::is_same<layout_type, first_order>::value ||
                ...
                "boost::numeric::basic_tensor template class only supports "
                "first- or last-order storage formats.");
...
protected:
  ...
  /** @brief Constructs a basic_tensor with an initializer list for
   * dynamic_extents
   *
   ...
   * basic_tensor
   */
  explicit inline basic_tensor(std::initializer_list<size_type> l)
   ...
   */
  explicit inline basic_tensor(extents_type const& s)
   ...
   */
  explicit inline basic_tensor(extents_type const& s, value_type const& i)
    : basic_tensor(s, container_tag{})
  {
    std::fill(begin(), end(), i);
  }

  /** @brief Constructs a basic_tensor with a \c shape and initiates it with
    ...
    : basic_tensor(s, container_tag{})
  {
    if (product(extents_) != a.size()) {
      throw std::runtime_error(
        "boost::numeric::ublas::basic_tensor(extents_type,array_type): array "
        "size mismatch with extents");
    }
    std::copy(a.begin(), a.end(), begin());
  }

  // /** @brief Constructs a basic_tensor using a shape tuple and initiates it
  // with a value.
  //  *
  ...
  // template<class U = E>
  // inline
  // basic_tensor (shape_t<typename extents_type::value_type, dynamic_rank>
  // const& e, const value_type &i, typename
...
public:
  /** @brief Constructs a basic_tensor with another basic_tensor with a
   * different layout
   *
    ...
    : basic_tensor(detail::retrieve_extents(expr), container_tag{})
  {
    static_assert(
      ...
      "Error in boost::numeric::ublas::basic_tensor: expression does not "
      "contain a basic_tensor. cannot retrieve shape.");
   ...
   * conform with this basic_tensor.
   *
  ...
  inline size_type size() const { return this->data_.size(); }

  /** @brief Returns the size of the basic_tensor */
  inline size_type size(size_type r) const { return this->extents_.at(r); }

  /** @brief Returns the number of dimensions/modes of the basic_tensor */
  inline size_type rank() const { return this->extents_.size(); }

  /** @brief Returns the number of dimensions/modes of the basic_tensor */
  inline size_type order() const { return this->extents_.size(); }

  /** @brief Returns the strides of the basic_tensor */
  inline strides_type const& strides() const { return this->strides_; }

  /** @brief Returns the extents of the basic_tensor */
  inline extents_type const& extents() const { return this->extents_; }


  /** @brief Returns a \c const reference to the container. */
  inline const_pointer data() const { return this->data_.data(); }

  /** @brief Returns a \c const reference to the container. */
  inline pointer data() { return this->data_.data(); }
   ...
   *  @param i zero-based index where 0 <= i < this->size()
   */
  inline reference operator[](size_type i) { return this->data_[i]; }
  ...
  inline const_reference at(size_type i, size_types... is) const
  {
    if constexpr (sizeof...(is) == 0)
      return this->data_[i];
    else
      return this->data_[detail::access<0ul>(
        size_type(0), this->strides_, i, std::forward<size_types>(is)...)];
  }
   ...
   */
  template <class... size_types>
  inline reference at(size_type i, size_types... is)
  {
    if constexpr (sizeof...(is) == 0)
      return this->data_[i];
    else {
      auto temp = detail::access<0ul>(
        size_type(0), this->strides_, i, std::forward<size_types>(is)...);
  ...
  inline decltype(auto) operator()(index::index_type<I> p,
                                   index_types... ps) const
  {
  ...
  /// reversed basic_tensor (i.e. end() of normal basic_tensor)
  inline const_reverse_iterator rbegin() const { return data_.rbegin(); }

  /// \brief Return a const reverse iterator before the first element of the
  /// reversed basic_tensor (i.e. end() of normal basic_tensor)
  inline const_reverse_iterator crbegin() const { return data_.crbegin(); }

  /// \brief Return a const reverse iterator on the end of the reverse
  /// basic_tensor (i.e. first element of the normal basic_tensor)
  inline const_reverse_iterator rend() const { return data_.rend(); }

  /// \brief Return a const reverse iterator on the end of the reverse
  /// basic_tensor (i.e. first element of the normal basic_tensor)
  inline const_reverse_iterator crend() const { return data_.crend(); }
