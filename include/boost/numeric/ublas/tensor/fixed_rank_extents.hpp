...
template <class ExtentsType, std::size_t N> struct basic_fixed_rank_extents
{
    ...
    static_assert(!std::numeric_limits<value_type>::is_signed,
                  "Static error in basic_fixed_rank_extents: type must be of type unsigned integer.");
    ...
    constexpr basic_fixed_rank_extents(std::initializer_list<value_type> li)
    {
        if (li.size() > _size)
        {
            throw std::out_of_range(
                "boost::numeric::ublas::basic_fixed_rank_extents(): initializer list size is greater than _size");
        }
        ...
        if (!valid(*this))
        {
            throw std::length_error("Error in basic_fixed_rank_extents::basic_fixed_rank_extents() : shape tuple is "
                                    "not a valid permutation: has zero elements.");
        }
    }
    ...
    inline constexpr void fill(value_type value)
    {
        _base.fill(value);
    }

    template <typename OtherExtentsType, std::enable_if_t<is_extents<OtherExtentsType>::value, int> = 0>
    constexpr basic_fixed_rank_extents(OtherExtentsType const &e)
    {
        std::copy_n(e.begin(), _size, _base.begin());
    }

    constexpr basic_fixed_rank_extents(base_type const &data) : _base(data)
    {
        if (!valid(*this))
        {
            throw std::length_error("Error in basic_fixed_rank_extents::basic_fixed_rank_extents() : shape tuple is "
    ...
    constexpr basic_fixed_rank_extents(base_type &&data) : _base(std::move(data))
    {
        if (!valid(*this))
        {
            throw std::length_error("Error in basic_fixed_rank_extents::basic_fixed_rank_extents() : shape tuple is "
    ...
    [[nodiscard]] inline constexpr base_type const &base() const
    {
        return _base;
    }
