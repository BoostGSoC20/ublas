...
public:
  ...
  static_assert(
    std::numeric_limits<typename base_type::value_type>::is_integer,
  ...
  basic_fixed_rank_strides(basic_fixed_rank_extents<T, N> const& s)
  {
    ...
    if (!valid(s))
      throw std::runtime_error(
        "Error in boost::numeric::ublas::basic_fixed_rank_strides() : shape is "
        "not valid.");
    ...
    if (this->size() < 2)
      throw std::runtime_error(
        "Error in boost::numeric::ublas::basic_fixed_rank_strides() : size of "
        "strides must be greater or equal 2.");


    if constexpr (std::is_same<layout_type, first_order>::value) {
      size_type k = 1ul, kend = this->size();
      for (; k < kend; ++k) _base[k] = _base[k - 1] * s[k - 1];
    }
    else {
  ...
  [[nodiscard]] inline constexpr reference back() { return _base[N - 1]; }

  [[nodiscard]] inline constexpr bool empty() const { return _base.empty(); }

  [[nodiscard]] inline constexpr size_type size() const { return _base.size(); }
