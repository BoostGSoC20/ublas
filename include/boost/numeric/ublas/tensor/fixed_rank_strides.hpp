...
template <class T, std::size_t N, class L> class basic_fixed_rank_strides
{
  public:
    ...
    static_assert(
        !std::numeric_limits<typename base_type::value_type>::is_signed,
        "Static error in boost::numeric::ublas::basic_fixed_rank_strides: type must be of type unsigned integer.");
    static_assert(
        std::is_same<L, first_order>::value || std::is_same<L, last_order>::value,
     ...
     */
    constexpr explicit basic_fixed_rank_strides() : _base{}
    {
    }
    ...
    basic_fixed_rank_strides(basic_fixed_rank_extents<T, N> const &s)
    {
        ...
        if (!valid(s))
            throw std::runtime_error(
                "Error in boost::numeric::ublas::basic_fixed_rank_strides() : shape is not valid.");

        if (is_vector(s) || is_scalar(s))
            return;

        if (this->size() < 2)
            throw std::runtime_error("Error in boost::numeric::ublas::basic_fixed_rank_strides() : size of strides "
    ...
    basic_fixed_rank_strides(basic_fixed_rank_strides const &l) : _base(l._base)
    {
    }

    basic_fixed_rank_strides(basic_fixed_rank_strides &&l) : _base(std::move(l._base))
    {
    }

    basic_fixed_rank_strides(base_type const &l) : _base(l)
    {
    }

    basic_fixed_rank_strides(base_type &&l) : _base(std::move(l))
    {
    }
