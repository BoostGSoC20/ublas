...
struct dynamic_tensor : public basic_tensor<dynamic_tensor<T, F>>
{
    ...
    dynamic_tensor(std::initializer_list<size_type> li) : super_type(std::move(li))
    {
    }

    constexpr dynamic_tensor(extents_type const &e, value_type const &i) : super_type(e, i)
    {
    }

    constexpr dynamic_tensor(extents_type const &e, array_type const &a) : super_type(e, a)
    {
    }

    constexpr dynamic_tensor(extents_type const &e) : super_type(e)
    {
    }

    constexpr dynamic_tensor(matrix_type const &v) : super_type(extents_type{v.size1(), v.size2()})
    {
        std::copy(v.data().begin(), v.data().end(), super_type::data_.begin());
    }

    constexpr dynamic_tensor(matrix_type &&v) : super_type(extents_type{v.size1(), v.size2()})
    {
    ...
    constexpr dynamic_tensor(vector_type &&v) : super_type(extents_type{v.size(), typename extents_type::value_type{1}})
    {
        std::move(v.data().begin(), v.data().end(), super_type::begin());
    }

    template <class OtherLayout>
    constexpr dynamic_tensor(const dynamic_tensor<value_type, OtherLayout> &expr) : super_type(expr)
    {
    }

    template <class derived_type>
    constexpr dynamic_tensor(const tensor_expression_type<derived_type> &expr) : super_type(expr)
    {
    }

    template <class derived_type>
    inline constexpr dynamic_tensor &operator=(const tensor_expression_type<derived_type> &expr)
    {
    ...
    template <class derived_type>
    constexpr dynamic_tensor(const matrix_expression_type<derived_type> &expr) : dynamic_tensor(matrix_type(expr))
    {
    }

    template <class derived_type>
    constexpr dynamic_tensor(const vector_expression_type<derived_type> &expr) : dynamic_tensor(vector_type(expr))
    {
    }

    template <class OtherTensor>
    constexpr dynamic_tensor(const basic_tensor<OtherTensor> &other) : super_type(other.extents())
    {
        static_assert(
    ...
    template <class OtherTensor>
    constexpr dynamic_tensor(basic_tensor<OtherTensor> &&other) : super_type(other.extents())
    {
        static_assert(
            is_valid_tensor_v<OtherTensor>,
...
template <typename V> dynamic_tensor(dynamic_extents<> const &, V const &)->dynamic_tensor<V, first_order>;
...
template <typename T, typename F> struct tensor_traits<dynamic_tensor<T, F>>
{
