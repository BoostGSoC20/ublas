...
template <class T> class basic_tensor;
...
template <class T, class D> struct has_tensor_types<T, tensor_expression<T, D>>
{
    static constexpr bool value = std::is_same<T, D>::value || has_tensor_types<T, D>::value;
};

template <class T, class EL, class ER, class OP> struct has_tensor_types<T, binary_tensor_expression<T, EL, ER, OP>>
{
    static constexpr bool value = std::is_same<T, EL>::value || std::is_same<T, ER>::value ||
                                  has_tensor_types<T, EL>::value || has_tensor_types<T, ER>::value;
};

template <class T, class E, class OP> struct has_tensor_types<T, unary_tensor_expression<T, E, OP>>
{
 ...
 *
 */
template <class TensorType>[[nodiscard]] constexpr auto &retrieve_extents(basic_tensor<TensorType> const &t)
{
 ...
 */
template <class T, class D>[[nodiscard]] constexpr auto &retrieve_extents(tensor_expression<T, D> const &expr)
{
    static_assert(
        detail::has_tensor_types<T, tensor_expression<T, D>>::value,
 ...
 * @note basic_tensor expression must be a binary tree with at least one basic_tensor type
 *
 * @returns extents of the (left and if necessary then right) child expression if it is a basic_tensor or extents of a
 * child of its (left and if necessary then right) child.
 */
template <class T, class EL, class ER, class OP>
[[nodiscard]] constexpr auto &retrieve_extents(binary_tensor_expression<T, EL, ER, OP> const &expr)
{
    static_assert(
...
[[nodiscard]] constexpr auto &retrieve_extents(unary_tensor_expression<T, E, OP> const &expr)
{
...
[[nodiscard]] inline constexpr auto all_extents_equal(basic_tensor<TensorType> const &t, Extents const &extents)
{
    static_assert(
        is_extents_v<Extents>,
...
[[nodiscard]] constexpr auto all_extents_equal(tensor_expression<T, D> const &expr, Extents const &extents)
{
    static_assert(
        is_extents_v<Extents>,
        "Error in boost::numeric::ublas::detail::all_extents_equal: extents passed should be of extents type.");
...
[[nodiscard]] constexpr auto all_extents_equal(binary_tensor_expression<T, EL, ER, OP> const &expr,
...
[[nodiscard]] constexpr auto all_extents_equal(unary_tensor_expression<T, E, OP> const &expr, Extents const &extents)
{
    static_assert(
        is_extents_v<Extents>,
        "Error in boost::numeric::ublas::detail::all_extents_equal: extents passed should be of extents type.");
    ...
    if constexpr (detail::has_tensor_types<tensor_type, tensor_expression<tensor_type, derived_type>>::value)
        if (!detail::all_extents_equal(expr, lhs.extents()))
            throw std::runtime_error(
                "Error in boost::numeric::ublas::basic_tensor: expression contains tensors with different shapes.");
 ...
 * \note Checks if shape of the basic_tensor matches those of all tensors within the expression.
 */
template <class tensor_type, class unary_fn> void eval(tensor_type &lhs, unary_fn const fn)
{
#pragma omp parallel for
    for (auto i = 0u; i < lhs.size(); ++i)
        fn(lhs(i));
