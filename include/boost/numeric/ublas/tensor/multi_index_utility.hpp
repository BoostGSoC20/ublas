...
template <class itype_left, class itype_right> struct has_index_impl<itype_left, itype_right>
{
...
template <class itype_left, class itype_right> struct has_index_impl<itype_left, std::tuple<itype_right>>
{
...
template <class itype, class... index_types> struct valid_multi_index_impl<std::tuple<itype, index_types...>>
{
...
template <class... itypes_right> struct number_equal_indexes_impl<std::tuple<>, std::tuple<itypes_right...>>
{
...
template <std::size_t r, std::size_t m, class itype, class ttype> struct index_position_impl
{
    static constexpr auto is_same =
        std::is_same<std::decay_t<itype>, std::decay_t<std::tuple_element_t<r, ttype>>>::value;
    static constexpr auto value = is_same ? r : index_position_impl<r + 1, m, itype, ttype>::value;
};

template <std::size_t m, class itype, class ttype> struct index_position_impl<m, m, itype, ttype>
{
...
template <class index_type, class tuple_type> struct index_position
{
    static constexpr auto value =
        detail::index_position_impl<0ul, std::tuple_size<tuple_type>::value, std::decay_t<index_type>,
...
{
    template <class array_type, class tuple_left, class tuple_right>
    static constexpr void run([[maybe_unused]] array_type &out, tuple_left const &, tuple_right const &,
 ...
 */
template <class tuple_left, class tuple_right> auto index_position_pairs(tuple_left const &lhs, tuple_right const &rhs)
{
...
{
    return std::make_pair(std::vector<std::size_t>{std::get<0>(std::get<R>(array)) + 1 ...},
                          std::vector<std::size_t>{std::get<1>(std::get<R>(array)) + 1 ...});
}

} // namespace detail

/** @brief array_to_vector converts a std::array of zero-based index position pairs into two std::vector of one-based
 * index positions
 *
 ...
 */
template <class pair_type, std::size_t N> constexpr auto array_to_vector(std::array<pair_type, N> const &array)
{
