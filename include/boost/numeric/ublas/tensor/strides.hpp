...
template <class LStrides, class RStrides, std::enable_if_t<is_strides_v<LStrides> && is_strides_v<RStrides>, int> = 0>
[[nodiscard]] inline constexpr bool operator==(LStrides const &lhs, RStrides const &rhs) noexcept
{
    static_assert(
        std::is_same_v<typename LStrides::value_type, typename RStrides::value_type>,
...
[[nodiscard]] inline constexpr auto access(std::vector<size_type> const &i, Stride const &w)
{
...
[[nodiscard]] constexpr auto access(std::size_t sum, Stride const &w, std::size_t i, size_types... is)
{
    static_assert(is_strides_v<Stride>,
                  "boost::numeric::ublas::detail::access() : invalid type, type should be a strides");
    sum += i * w[r];
