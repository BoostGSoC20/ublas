 ...
 *
 **/
template <class T, class E> struct tensor_expression : public ublas_expression<E>
{
...
struct binary_tensor_expression : public tensor_expression<T, binary_tensor_expression<T, EL, ER, OP>>
{
    ...
    binary_tensor_expression(binary_tensor_expression &&l) : el(l.el), er(l.er), op(l.op)
    {
    }
...
auto make_binary_tensor_expression(tensor_expression<T1, EL> const &el, tensor_expression<T2, ER> const &er, OP op)
{
    static_assert(std::is_same_v<typename T1::value_type, typename T2::value_type>,
                  "boost::numeric::ublas::make_binary_tensor_expression(T1,T2) : LHS tensor and RHS tensor should have "
                  "same value type");
...
struct unary_tensor_expression : public tensor_expression<T, unary_tensor_expression<T, E, OP>>
{
    ...
    unary_tensor_expression(unary_tensor_expression &&l) : e(l.e), op(op.l)
    {
    }
...
// \brief helper function to simply instantiation of lambda proxy class
template <class T, class E, class OP> auto make_unary_tensor_expression(tensor_expression<T, E> const &e, OP op)
{
    return unary_tensor_expression<T, E, OP>(e(), op);
}

template <class T, class E, class OP> auto make_unary_tensor_expression(matrix_expression<E> const &e, OP op)
{
    return unary_tensor_expression<T, E, OP>(e(), op);
}

template <class T, class E, class OP> auto make_unary_tensor_expression(vector_expression<E> const &e, OP op)
{
    return unary_tensor_expression<T, E, OP>(e(), op);
}
