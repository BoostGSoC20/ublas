...
struct fixed_rank_tensor : public basic_tensor<fixed_rank_tensor<T, R, F>>
{
    ...
    constexpr fixed_rank_tensor(extents_type const &e, value_type const &i) : super_type(e, i)
    {
    }

    constexpr fixed_rank_tensor(extents_type const &e, array_type const &a) : super_type(e, a)
    {
    }

    constexpr fixed_rank_tensor(extents_type const &e) : super_type(e)
    {
    }

    fixed_rank_tensor(std::initializer_list<size_type> li) : super_type(std::move(li))
    {
    }

    constexpr fixed_rank_tensor(matrix_type const &v) : super_type(extents_type{v.size1(), v.size2()})
    {
    ...
    constexpr fixed_rank_tensor(matrix_type &&v) : super_type(extents_type{v.size1(), v.size2()})
    {
    ...
    template <class derived_type>
    constexpr fixed_rank_tensor(const tensor_expression_type<derived_type> &expr) : super_type(expr)
    {
    }

    template <class derived_type>
    inline constexpr fixed_rank_tensor &operator=(const tensor_expression_type<derived_type> &expr)
    {
    ...
    template <class derived_type>
    constexpr fixed_rank_tensor(const matrix_expression_type<derived_type> &expr) : fixed_rank_tensor(matrix_type(expr))
    {
    }

    template <class derived_type>
    constexpr fixed_rank_tensor(const vector_expression_type<derived_type> &expr) : fixed_rank_tensor(vector_type(expr))
    {
    }

    template <class OtherTensor>
    constexpr fixed_rank_tensor(const basic_tensor<OtherTensor> &other) : super_type(other.extents())
    {
        static_assert(is_valid_tensor_v<OtherTensor>, "boost::numeric::ublas::fixed_rank_tensor( const OtherTensor& ) "
                                                      ": The OtherTensor should have a tensor type");

        static_assert(
            std::is_same_v<value_type, typename OtherTensor::value_type>,
            "boost::numeric::ublas::fixed_rank_tensor( const OtherTensor& ) : The tensor should have same value type");

        if (other.extents().size() != R)
        {
            throw std::runtime_error("boost::numeric::ublas::static_tensor( const OtherTensor& ) : The tensor should "
                                     "have equal number of extents");
        }
    ...
    template <class OtherTensor>
    constexpr fixed_rank_tensor(basic_tensor<OtherTensor> &&other) : super_type(other.extents())
    {
        static_assert(
            is_valid_tensor_v<OtherTensor>,
        ...
        if (other.extents().size() != R)
        {
            throw std::runtime_error("boost::numeric::ublas::static_tensor( OtherTensor&& ) : The tensor should have "
                                     "equal number of extents");
        }
...
template <typename V, typename E> fixed_rank_tensor(E const &, V const &)->fixed_rank_tensor<V, E::_size, first_order>;

template <typename E> fixed_rank_tensor(E const &)->fixed_rank_tensor<float, E::_size, first_order>;
...
template <typename T, std::size_t R, typename F> struct tensor_traits<fixed_rank_tensor<T, R, F>>
{
