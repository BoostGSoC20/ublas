...
BOOST_AUTO_TEST_SUITE(test_tensor_algorithms,
                      *boost::unit_test::depends_on("test_extents") * boost::unit_test::depends_on("test_strides"))
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_algorithms_transform, value, test_types2, fixture)
{
    ...
    for (auto const &n : extents)
    {
        ...
        ublas::transform(n.size(), n.data(), b.data(), wb.data(), a.data(), wa.data(),
                         [](value_type const &a) { return a + value_type(1); });
        ublas::transform(n.size(), n.data(), c.data(), wc.data(), b.data(), wb.data(),
                         [](value_type const &a) { return a - value_type(1); });

        using size_type = typename ublas::strides_t<ublas::dynamic_extents<>, ublas::first_order>::value_type;

        size_type zero = 0;
        ublas::transform(zero, n.data(), c.data(), wc.data(), b.data(), wb.data(),
                         [](value_type const &a) { return a + value_type(1); });
        ...
        BOOST_CHECK_THROW(ublas::transform(n.size(), n.data(), c0, wb.data(), a.data(), wa.data(),
                                           [](value_type const &a) { return a + value_type(1); }),
                          std::runtime_error);
        BOOST_CHECK_THROW(ublas::transform(n.size(), n.data(), b.data(), s0, a.data(), wa.data(),
                                           [](value_type const &a) { return a + value_type(1); }),
                          std::runtime_error);
        BOOST_CHECK_THROW(ublas::transform(n.size(), p0, b.data(), wb.data(), a.data(), wa.data(),
                                           [](value_type const &a) { return a + value_type(1); }),
                          std::runtime_error);
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_algorithms_transform_exceptions, value, test_types2, fixture)
{
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::transform(n.size(), n.data(), c.data(), wc.data(), a, wa.data(),
                                             [](value_type const &a) { return a + value_type(1); }),
                            std::runtime_error);
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::transform(n.size(), n.data(), c, wc.data(), a, wa.data(),
                                             [](value_type const &a) { return a + value_type(1); }),
                            std::runtime_error);
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::transform(n.size(), n.data(), c, wc.data(), a.data(), wa.data(),
                                             [](value_type const &a) { return a + value_type(1); }),
                            std::runtime_error);
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::transform(n.size(), n.data(), c.data(), wc.data(), a.data(), wa,
                                             [](value_type const &a) { return a + value_type(1); }),
                            std::runtime_error);
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::transform(n.size(), n.data(), c.data(), wc, a.data(), wa.data(),
                                             [](value_type const &a) { return a + value_type(1); }),
                            std::runtime_error);
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::transform(n.size(), m, c.data(), wc.data(), a.data(), wa.data(),
                                             [](value_type const &a) { return a + value_type(1); }),
                            std::runtime_error);
    }
}
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_algorithms_accumulate, value, test_types2, fixture)
{
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_CHECK_THROW(
            ublas::accumulate(n.size(), n.data(), c0, wa.data(), v, [](auto const &l, auto const &r) { return l + r; }),
            std::runtime_error);
        BOOST_CHECK_THROW(
            ublas::accumulate(n.size(), n.data(), a.data(), p0, v, [](auto const &l, auto const &r) { return l + r; }),
            std::runtime_error);
        BOOST_CHECK_THROW(
            ublas::accumulate(n.size(), p0, a.data(), wa.data(), v, [](auto const &l, auto const &r) { return l + r; }),
            std::runtime_error);
    }
}
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_algorithms_accumulate_exceptions, value, test_types2, fixture)
{
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::accumulate(n.size(), n.data(), a, wa.data(), value_type{0},
    ...
    for (auto const &n : extents)
    {
        ...
        auto wa = ublas::strides_t<ublas::dynamic_extents<>, ublas::first_order>(n);
        size_t p = 0u;
        BOOST_CHECK_EQUAL(ublas::accumulate(p, n.data(), a.data(), wa.data(), value_type{0},
                                            [](value_type const &a, value_type const &b) { return a + b; }),
                          value_type{0});
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::accumulate(n.size(), m, a.data(), wa.data(), value_type{0},
                                              [](value_type const &a, value_type const &b) { return a + b; }),
                            std::runtime_error);
    }
}
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_algorithms_trans, value, test_types, fixture)
{
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_CHECK_THROW(ublas::trans(p, s0, pi.data(), c2.data(), wc.data(), a.data(), wa.data()),
                          std::runtime_error);
...
BOOST_FIXTURE_TEST_CASE_TEMPLATE(test_tensor_algorithms_trans_exceptions, value, test_types, fixture)
{
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::trans(p, nc.data(), pi.data(), a, wa.data(), c.data(), wc.data()),
                            std::runtime_error);
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::trans(p, nc.data(), pi.data(), c.data(), wc.data(), a, wa.data()),
                            std::runtime_error);
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::trans(p, nc.data(), pi.data(), c.data(), wc, a.data(), wa.data()),
                            std::runtime_error);
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::trans(p, nc.data(), pi.data(), c.data(), wc.data(), a.data(), wa),
                            std::runtime_error);
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::trans(p, nc.data(), pi, c.data(), wc.data(), a.data(), wa.data()),
                            std::runtime_error);
    }
    ...
    for (auto const &n : extents)
    {
        ...
        BOOST_REQUIRE_THROW(ublas::trans(p, nc, pi.data(), c.data(), wc.data(), a.data(), wa.data()),
                            std::runtime_error);
    }
