...
template <size_t... E> using extents = boost::numeric::ublas::basic_static_extents<unsigned, E...>;
...
struct fixture
{
    ...
    std::tuple<extents<1>, extents<1, 1>, extents<1, 1, 1>, extents<1, 1, 1, 1>> scalars;

    std::tuple<extents<1, 2>, extents<1, 3, 1>, extents<1, 4, 1, 1>, extents<5, 1, 1, 1, 1>, extents<6, 1, 1, 1, 1, 1>>
        vectors;

    std::tuple<extents<2, 3>, extents<3, 2, 1>, extents<4, 4, 1, 1>, extents<6, 6, 1, 1, 1, 1>> matrices;

    std::tuple<extents<3, 3, 3>, extents<4, 4, 4, 1>, extents<5, 5, 5, 1, 1>, extents<6, 6, 6, 1, 1, 1>,
               extents<6, 6, 1, 1, 1, 6>>
        tensors;
};

BOOST_FIXTURE_TEST_CASE(test_static_extents_product, fixture,
                        *boost::unit_test::label("static_extents") * boost::unit_test::label("product"))
{
...
BOOST_FIXTURE_TEST_CASE(test_static_extents, fixture,
                        *boost::unit_test::label("static_extents") * boost::unit_test::label("query"))
{
...
BOOST_FIXTURE_TEST_CASE(test_static_extents_valid, fixture,
                        *boost::unit_test::label("static_extents") * boost::unit_test::label("valid"))
{
    using namespace boost::numeric::ublas;
    for_each_tuple(rank_0_extents, [](auto const &, auto &e) {
        BOOST_CHECK(!valid(e));
...
BOOST_FIXTURE_TEST_CASE(test_static_extents_comparsion_operator, fixture,
                        *boost::unit_test::label("static_extents") * boost::unit_test::label("comparsion_operator"))
{
    ...
    for_each_tuple(rank_0_extents, [&](auto const &, auto const &e1) {
        for_each_tuple(rank_1_extents,
                       [&](auto const &, auto const &e2) { BOOST_CHECK(compare_extents(e1, e2) == (e1 == e2)); });
    });
    ...
    for_each_tuple(rank_1_extents, [&](auto const &, auto const &e1) {
        for_each_tuple(rank_2_extents,
                       [&](auto const &, auto const &e2) { BOOST_CHECK(compare_extents(e1, e2) == (e1 == e2)); });
    });

    for_each_tuple(scalars, [&](auto const &, auto const &e1) {
        for_each_tuple(scalars,
                       [&](auto const &, auto const &e2) { BOOST_CHECK(compare_extents(e1, e2) == (e1 == e2)); });
    });

    for_each_tuple(scalars, [&](auto const &, auto const &e1) {
        for_each_tuple(vectors,
                       [&](auto const &, auto const &e2) { BOOST_CHECK(compare_extents(e1, e2) == (e1 == e2)); });
    });

    for_each_tuple(scalars, [&](auto const &, auto const &e1) {
        for_each_tuple(matrices,
                       [&](auto const &, auto const &e2) { BOOST_CHECK(compare_extents(e1, e2) == (e1 == e2)); });
    });

    for_each_tuple(scalars, [&](auto const &, auto const &e1) {
        for_each_tuple(tensors,
                       [&](auto const &, auto const &e2) { BOOST_CHECK(compare_extents(e1, e2) == (e1 == e2)); });
    });
}

BOOST_FIXTURE_TEST_CASE(test_static_extents_squeeze, fixture,
                        *boost::unit_test::label("static_extents") * boost::unit_test::label("squeeze"))
{
