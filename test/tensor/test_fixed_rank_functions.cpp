...
struct fixture
{
    template <size_t R> using fixed_rank_extents_type = boost::numeric::ublas::dynamic_extents<R>;
    ...
    std::tuple<fixed_rank_extents_type<2>, fixed_rank_extents_type<2>, fixed_rank_extents_type<3>,
               fixed_rank_extents_type<3>, fixed_rank_extents_type<4>>
        fixed_rank_extents{
            {1, 1},      // 1
            {2, 3},      // 2
            {2, 3, 1},   // 3
    ...
    for_each_tuple(fixed_rank_extents, [&](auto const &, auto &n) {
        ...
        for (auto f = 0ul; f < fac; ++f)
        {
            ...
            for (auto q = 0ul; q <= pa; ++q)
            {
                ...
                std::transform(phia.begin(), phia.end(), phib.begin(), [&pi](std::size_t i) { return pi.at(i - 1); });
